...
function DashboardProfileEdit() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/dashboard/edit-profile", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      document.getElementById("avatar").value = ""; // Clear file input
      navigate("/dashboard");
    } catch (error) {
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group text-center mb-4">
                ...
                <div>
                  ...
                  {formData.avatar && <small className="d-block mt-1 text-muted">{formData.avatar.name}</small>}
                </div>
              </div>
              ...
              <div className="form-group mb-3">
                <label>Email:</label>
                <input type="email" className="form-control" value={formData.email} disabled required />
              </div>
                ...
                <div>
                  {hobbiesList.map((hobby, index) => (
                    <label key={index} className="form-check-label me-3">
                      <input className="form-check-input me-1" type="checkbox" name="hobbies" value={hobby} checked={formData.hobbies.includes(hobby)} onChange={handleHobbyChange} />
                      {hobby}
                    </label>
                  ))}
                </div>
